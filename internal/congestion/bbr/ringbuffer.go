...
func (r *RingBuffer[T]) Offset(index int) *T {
	if r.Empty() || index >= r.Len() {
		panic("ringbuffer: offset from invalid index")
	}
...
func (r *RingBuffer[T]) Front() *T {
	if r.Empty() {
		panic("ringbuffer: front from an empty queue")
	}
...
func (r *RingBuffer[T]) Back() *T {
	if r.Empty() {
		panic("ringbuffer: back from an empty queue")
	}
	return r.Offset(r.Len() - 1)
}

// grow increases the maximum size of the queue.
