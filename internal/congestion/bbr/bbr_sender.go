...
import (
	...
	"github.com/zijiren233/gwst/internal/congestion/common"
)
...
// Profile defines the BBR congestion control behavior profile.
type Profile string
...
// ParseProfile parses a profile string into a Profile constant.
...
type bbrSender struct {
	...
	// BBR profile controlling gain parameters and algorithm features
	profile Profile
	...
	// The gain used for the congestion window during PROBE_BW. Latched from quic_bbr_cwnd_gain flag.
	congestionWindowGainConstant float64
	...
	// When overshooting is detected, do not drop pacing_rate_ below this value / min_rtt.
...
func (b *bbrSender) OnCongestionEventEx(priorInFlight congestion.ByteCount, eventTime monotime.Time, ackedPackets []congestion.AckedPacketInfo, lostPackets []congestion.LostPacketInfo) {
	...
	sample := b.sampler.OnCongestionEvent(eventTime,
		...
		b.maxBandwidth.GetBest(), infBandwidth, b.roundTripCount)
...
func (b *bbrSender) PacingRate() Bandwidth {
	if b.pacingRate == 0 {
		return Bandwidth(b.highGain *
			float64(BandwidthFromDelta(b.initialCongestionWindow, b.getMinRtt())))
	}
	return b.pacingRate
}
...
// setHighGain sets the pacing gain used in STARTUP. Must be greater than 1.
...
// setHighCwndGain sets the CWND gain used in STARTUP. Must be greater than 1.
...
// setDrainGain sets the gain used in DRAIN. Must be less than 1.
...
// getMinRtt returns the current estimate of the RTT of the connection.
...
// maybeExitStartupOrDrain transitions from STARTUP to DRAIN and from DRAIN to PROBE_BW if appropriate.
	...
	if b.detectOvershooting {
		...
		if b.pacingRate > targetRate && b.bytesLostWhileDetectingOvershooting > 0 {
			if b.hasNoAppLimitedSample ||
				b.bytesLostWhileDetectingOvershooting*congestion.ByteCount(b.bytesLostMultiplierWhileDetectingOvershooting) >
					b.initialCongestionWindow {
				b.pacingRate = max(targetRate,
					BandwidthFromDelta(b.cwndToCalculateMinPacingRate, b.rttStats.MinRTT()))
...
func bdpFromRttAndBandwidth(rtt time.Duration, bandwidth Bandwidth) congestion.ByteCount {
	return congestion.ByteCount(rtt) * congestion.ByteCount(bandwidth) /
		congestion.ByteCount(BytesPerSecond) / congestion.ByteCount(time.Second)
}

// GetInitialPacketSize returns the initial packet size based on the address type.
